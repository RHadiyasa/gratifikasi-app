...
export async function POST(req) {
  try {
    const { sheetUrl, sheetName, unitName, eselon1, target } = await req.json();
    if (!sheetUrl?.trim()) {
    ...
    const rows = allDataRows.map((row) => {
      ...
      return { id, bukti, link: hasLink ? link : null, status, verdict: isChecked ? vr.result : null, verdictColor, reviu: isChecked ? vr.reviu : null, supervisi: vr?.supervisi ?? null, tglCek: isChecked ? vr.tglCek : null };
    });
    ...
    applyHeader(ws1.addRow(["ID", "Nama Bukti Data", "Link Drive", "Hasil AI", "Catatan Reviu", "Tgl Cek", "Status"]));
    ...
    ws2.addRow([]);
    applyHeader(ws2.addRow(["ID", "Nama Bukti Data", "Link Drive", "Hasil AI", "Catatan Reviu", "Tgl Cek", "Keterangan"]));
