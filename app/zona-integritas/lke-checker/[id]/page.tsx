...
type TabKey = "semua" | "bermasalah" | "unchecked" | "no_link" | "revisi" | "checked";
...
const STATUS_CONFIG: Record<RowStatus, { label: string; color: string; icon: React.ReactNode }> = {
  checked:   { label: "Sudah Dicek", color: "text-green-600 dark:text-green-400", icon: <CheckCircle2 size={12} /> },
  unchecked: { label: "Belum Dicek", color: "text-amber-500",                    icon: <Clock size={12} /> },
  no_link:   { label: "Tanpa Link",  color: "text-default-400",                   icon: <Link2Off size={12} /> },
  revisi:    { label: "Revisi",      color: "text-blue-500",                      icon: <RotateCw size={12} /> },
};

const VERDICT_STYLE: Record<string, string> = {
  HIJAU:  "bg-green-500/10 text-green-600 dark:text-green-400 border border-green-200 dark:border-green-800",
  KUNING: "bg-amber-500/10 text-amber-600 border border-amber-200 dark:border-amber-800",
...
export default function UnitDetailPage() {
  ...
  // ── Load detail ──
  const loadDetail = useCallback(async (sName?: string) => {
    if (!unit?.link) return;
    setLoading(true);
    setDetailError(null);
    try {
      const res  = await fetch("/api/zi/detail", {
        ...
        body:    JSON.stringify({ sheetUrl: unit.link, sheetName: sName ?? sheetName }),
      });
  ...
  const filtered = rows.filter((r) => {
    let matchTab = true;
    if (activeTab === "bermasalah") matchTab = r.verdictColor === "KUNING" || r.verdictColor === "MERAH" || r.status === "revisi" || r.status === "unchecked" || r.status === "no_link";
    else if (activeTab === "checked") matchTab = r.verdictColor === "HIJAU";
    else if (activeTab !== "semua")  matchTab = r.status === activeTab;
    const matchSearch = !search || r.id.includes(search) || r.bukti.toLowerCase().includes(search.toLowerCase());
    return matchTab && matchSearch;
  });

  const tabCount = (key: TabKey) => {
    if (key === "semua")      return rows.length;
    if (key === "bermasalah") return rows.filter((r) => r.verdictColor === "KUNING" || r.verdictColor === "MERAH" || r.status === "revisi" || r.status === "unchecked" || r.status === "no_link").length;
    if (key === "checked")    return rows.filter((r) => r.verdictColor === "HIJAU").length;
  ...
  if (unitError) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-8">
        <Button variant="light" startContent={<ArrowLeft size={15} />} onPress={() => router.back()} className="mb-4">Kembali</Button>
  ...
  if (!unit) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-8 flex items-center justify-center py-32 text-default-400 gap-2">
    ...
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="rounded-xl border border-default-200 px-4 py-3 space-y-2">
        <div className="flex justify-between text-xs text-default-500">
          ...
          <span className="font-mono tabular-nums">{unit.checked_count} / {unit.total_data || "?"}</span>
        </div>
      ...
      {summary && (
        <div className="grid grid-cols-4 sm:grid-cols-8 gap-2">
          {[
            ...
            { label: "✅ Sesuai",    value: sesuai,            cls: "text-green-600 dark:text-green-400" },
            { label: "⚠️ Sebagian",  value: sebagian,          cls: "text-amber-500" },
            { label: "❌ Tidak",     value: tidak,             cls: "text-red-500" },
            { label: "Belum Dicek",  value: summary.unchecked, cls: "text-default-500" },
            { label: "Tanpa Link",   value: summary.noLink,    cls: "text-default-400" },
            { label: "Revisi",       value: summary.revisi,    cls: "text-blue-500" },
            { label: "Progress",     value: `${summary.total > 0 ? Math.round(summary.checked / summary.total * 100) : 0}%`, cls: "text-primary" },
          ].map((s) => (
            <div key={s.label} className="text-center rounded-xl bg-default-50 dark:bg-default-100 p-2.5">
              <p className={`text-lg font-bold tabular-nums ${s.cls}`}>{s.value}</p>
              <p className="text-[10px] text-default-500 mt-0.5 leading-tight">{s.label}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {loading && !summary && (
        <div className="flex items-center justify-center py-20 text-default-400 gap-2 text-sm">
          <Loader2 size={16} className="animate-spin" /> Memuat data dari sheet...
        </div>
      )}
          ...
          <div className="flex gap-1.5 flex-wrap">
            {TABS.map((t) => {
              ...
              return (
                ...
                >
                  <span className={activeTab !== t.key ? (t.color ?? "") : ""}>{t.label}</span>
                  <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-mono ${activeTab === t.key ? "bg-white/20" : "bg-default-200"}`}>
                    {count}
                  </span>
                </button>
              );
          ...
          <div className="rounded-xl border border-default-200 overflow-hidden">
            <table className="w-full text-xs">
              <thead className="bg-default-50 border-b border-default-200">
                <tr>
                  <th className="text-left py-2.5 px-3 font-medium text-default-500 w-12">ID</th>
                  <th className="text-left py-2.5 px-3 font-medium text-default-500">Nama Bukti Data</th>
                  <th className="text-left py-2.5 px-3 font-medium text-default-500 w-28">Hasil AI</th>
                  <th className="text-left py-2.5 px-3 font-medium text-default-500 w-24">Status</th>
                  <th className="text-left py-2.5 px-3 font-medium text-default-500 w-20">Tgl Cek</th>
                  <th className="text-center py-2.5 px-3 font-medium text-default-500 w-10">Link</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-default-100">
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="py-14 text-center text-default-400">
                    ...
                    return (
                      <Fragment key={row.id}>
                        <tr
                          ...
                          onClick={() => row.reviu && setExpandedId(isExpanded ? null : row.id)}
                        >
                          <td className="py-2.5 px-3 font-mono font-bold text-default-700">{row.id}</td>
                          <td className="py-2.5 px-3">
                            <div className="flex items-center gap-1.5">
                              <span className="truncate max-w-[260px]" title={row.bukti}>
                                {row.bukti || <span className="text-default-300 italic">—</span>}
                              </span>
                              {row.reviu && (
                                isExpanded
                                  ? <ChevronUp size={11} className="text-default-400 shrink-0" />
                                  : <ChevronDown size={11} className="text-default-400 shrink-0" />
                              )}
                            </div>
                          </td>
                          <td className="py-2.5 px-3">
                            {row.verdictColor ? (
                              <span className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${VERDICT_STYLE[row.verdictColor]}`}>
                                {row.verdict}
                              </span>
                            ) : (
                          ...
                          <td className="py-2.5 px-3 text-center">
                            {row.link ? (
                              ...
                              <Link2Off size={12} className="text-default-300 inline" />
                            )}
                          </td>
                        </tr>
                          ...
                          <tr className="bg-default-50">
                            ...
                            <td colSpan={5} className="px-3 pb-3 pt-1">
                              <div className="rounded-lg bg-default-100 dark:bg-default-50 px-3 py-2 text-xs text-default-600 leading-relaxed">
                                <p className="font-semibold text-default-500 mb-1 text-[10px] uppercase tracking-wide">Catatan Reviu</p>
