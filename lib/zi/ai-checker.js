...
const QC_SAMPLE_RATE = 0.08; // 8% random baseline sampling

/**
 * Determines if this item should be QC-sampled via Sonnet.
 * Triggers on: high-score-low-confidence, Layer-0-vs-AI score conflict,
 * template/generic content flags, or random baseline sampling.
 */
export function shouldSampleForQC(heuristic, aiCheck, finalScore) {
  // False positive risk: high score but AI was unsure
  if (finalScore >= 70 && aiCheck.confidence === "low") return true;

  // Conflict: heuristic and AI disagree significantly
  const hScore = heuristic?.heuristicScore || 0;
  if (Math.abs(hScore - finalScore) > 35) return true;

  // Content quality flags from Layer 3
  if (aiCheck.isTemplate || aiCheck.hasGenericContent) return true;

  // Random baseline
  return Math.random() < QC_SAMPLE_RATE;
}
