...
// Tahun periode penilaian LKE — dipakai untuk validasi tahun data dukung
const TAHUN_PENILAIAN = new Date().getFullYear();

// ── Perakit konten teks (hasil ekstraksi/OCR seluruh halaman) ─────────────────

const MAX_TOTAL_CONTENT_CHARS = 100000;

function buildTextContents(fileContents, contentFiles) {
  const snippets = [];
  let total = 0;
  let skippedCount = 0;
  if (!fileContents) return { contentText: "", readCount: 0, skippedCount };
  ...
  for (let i = 0; i < fileContents.length; i++) {
    ...
    const chunk = `=== ${name} ===\n${c.trim()}`;
    if (total + chunk.length > MAX_TOTAL_CONTENT_CHARS) {
      skippedCount++;
      continue;
    }
    total += chunk.length;
  ...
  return { contentText: snippets.join("\n\n"), readCount: snippets.length, skippedCount };
}
...
export function checkByName(files, standarText) {
  ...
  // Mode ketat: kecocokan nama file TIDAK pernah memfinalkan verdict "Sesuai" —
  // tahun dan kelengkapan dokumen hanya bisa diverifikasi dari isi, sehingga
  // heuristik hanya menjadi sinyal routing/QC dan proses selalu lanjut ke AI.
  return {
    skip: false,
    heuristicScore,
    ambiguity,
...
// ── Layer 1: AI nama file vs standar (DeepSeek) ──────────────────────────────

const NAMES_STANDAR_SYSTEM =
  `Kamu adalah validator dokumen Zona Integritas (ZI) WBK/WBBM Kementerian ESDM.
Penilaianmu KETAT — penilaian tahap ini hanya menyaring; kesesuaian akhir ditentukan oleh verifikasi isi dokumen.
...
- Jika nama file mengandung kata kunci relevan dari standar, anggap dokumen kemungkinan ada
- Jangan mengarang kelengkapan: jika sebagian jenis dokumen yang diminta standar tidak tampak dari nama file, masukkan ke dokumenKurang
- Tahun dokumen TIDAK dapat dipastikan dari nama file — jangan beri confidence "high" hanya karena nama cocok
...
// ── Layer 1b: AI nama file vs PANRB (DeepSeek) ───────────────────────────────
...
INSTRUKSI:
- Nilai kesesuaian HANYA dari nama file dan path subfolder — JANGAN menunggu isi dokumen
- Penilaian KETAT mengacu kriteria PANRB — nilai apakah nama file mencerminkan pemenuhan kriteria, bukan sekadar upaya
- Jangan mengarang kelengkapan: dokumen yang dituntut kriteria namun tidak tampak dari nama file masuk ke dokumenKurang
- Tahun dokumen TIDAK dapat dipastikan dari nama file — jangan beri confidence "high" hanya karena nama cocok
...
// ── Layer 3: Konten vs standar (DeepSeek, teks hasil ekstraksi/OCR) ──────────

const CHECKER_SYSTEM =
  `Kamu adalah validator dokumen Zona Integritas (ZI) WBK/WBBM Kementerian ESDM.
Penilaianmu KETAT dan mengacu pada KRITERIA PANRB — keberadaan file BUKAN bukti kesesuaian; isi dokumenlah yang dinilai.

TAHUN PERIODE PENILAIAN: ${TAHUN_PENILAIAN}

INSTRUKSI PENILAIAN (KETAT):
- Daftar file sudah termasuk semua file di dalam subfolder (path lengkap ditampilkan)
- ISI DOKUMEN disertakan sebagai teks hasil ekstraksi/OCR SELURUH HALAMAN — baca secara detail
- KESESUAIAN TAHUN: periksa tahun/tanggal pada setiap dokumen. Data dukung harus relevan dengan
  periode penilaian (tahun ${TAHUN_PENILAIAN}, atau ${TAHUN_PENILAIAN - 1} bila kriteria menuntut dokumen tahun sebelumnya).
  Dokumen bertahun lebih lama dari itu TIDAK memenuhi — verdict TIDAK BOLEH "Sesuai", dan sebutkan
  tahun dokumen yang ditemukan di catatan
- KELENGKAPAN: seluruh jenis dokumen yang diminta standar/kriteria harus ada DAN terisi.
  Jika ada yang kurang, verdict maksimal "Sebagian Sesuai" dan setiap kekurangan WAJIB ditulis di dokumenKurang
- Verdict "Sesuai" (skor ≥70) HANYA jika: dokumen lengkap, tahun sesuai periode penilaian,
  dan isinya secara substansial menjawab standar/kriteria
- Jangan menaikkan skor karena "sudah ada upaya" — nilai bukti apa adanya
- catatan WAJIB diisi spesifik: sebutkan masalah tahun/kelengkapan yang ditemukan
  (contoh: "SK masih tahun 2023, belum diperbarui; daftar hadir sosialisasi tidak ditemukan"),
  atau konfirmasi singkat bila semuanya sesuai
- Teks hasil OCR bisa mengandung kesalahan pembacaan minor — jangan menghukum typo OCR,
  namun tetap teliti membaca tahun, nomor, dan substansi dokumen
...
Jawab HANYA format JSON (tanpa markdown):
{"score":<0-100>,"confidence":"<low|medium|high>","verdict":"<Sesuai|Sebagian Sesuai|Tidak Sesuai>","dokumenAda":[],"dokumenKurang":[],"catatan":"<masalah tahun/kelengkapan yang ditemukan, atau konfirmasi singkat>","basedOn":"<nama_file|konten|keduanya>","isTemplate":<true|false>,"hasGenericContent":<true|false>}`;
...
export async function checkWithAIContent(files, fileContents, standarText, id, readableFiles) {
  ...
  const { contentText, readCount, skippedCount } = buildTextContents(fileContents, contentFiles);

  const fileList = files.map((f) => `- ${f.path || f.name} (${f.mimeType || "unknown"})`).join("\n");

  let contentSection = "";
  if (readCount > 0) {
    contentSection = `\nISI DOKUMEN (teks hasil ekstraksi/OCR seluruh halaman, ${readCount} file):\n${contentText}`;
    if (skippedCount > 0) {
      contentSection += `\n(${skippedCount} file lain tidak disertakan karena melebihi batas ukuran konten)`;
    }
  } else {
...
// ── Layer 1 (Auditor): Folder + Narasi vs PANRB (DeepSeek) ───────────────────
...
- Nama file/narasi mengindikasikan dokumen tahun lama (di bawah ${TAHUN_PENILAIAN - 1}) — catat sebagai temuan

CATATAN PENTING: skormu adalah penyaring awal — item dengan skor ≥40 akan diverifikasi lebih lanjut
dari ISI dokumen (kesesuaian tahun dan kelengkapan). Jangan memberi skor ≥40 jika bukti jelas tidak relevan,
dan jangan menahan skor item yang layak diverifikasi.
...
// ── Layer 4: Deep Review DeepSeek (rescue + QC sampling) ─────────────────────
...
- ISI DOKUMEN adalah teks hasil ekstraksi/OCR seluruh halaman — baca secara detail
- TAHUN PERIODE PENILAIAN: ${TAHUN_PENILAIAN}. Data dukung harus relevan dengan periode ini
  (tahun ${TAHUN_PENILAIAN}, atau ${TAHUN_PENILAIAN - 1} bila kriteria menuntut dokumen tahun sebelumnya).
  Dokumen bertahun lebih lama TIDAK memenuhi — revisedVerdict tidak boleh "Sesuai", sebutkan tahunnya di review
- KELENGKAPAN: seluruh dokumen yang dituntut kriteria harus ada dan terisi; jika kurang,
  revisedVerdict maksimal "Sebagian Sesuai" dan kekurangan disebutkan eksplisit di review/auditFinding
- Teks hasil OCR bisa mengandung kesalahan pembacaan minor — jangan menghukum typo OCR
- Jika konten dokumen relevan, lengkap, dan tahunnya sesuai, berikan skor yang layak
...
/**
 * Mode ketat: setiap item yang memiliki file WAJIB diverifikasi isinya —
 * kesesuaian tahun dan kelengkapan data dukung hanya bisa dinilai dari konten,
 * sehingga skor nama file tidak pernah cukup untuk memfinalkan "Sesuai".
 *
...
export function confidenceRouting(heuristic, aiCheck, existCheck, bobot = 0) {
  ...
  return { needsContentCheck: true, reason: "strict_verification" };
}

// ── QC Sampling ───────────────────────────────────────────────────────────────

/**
 * Determines if this item should be QC-sampled via deep review.
...
export function calculateFinalVerdict(existCheck, aiCheck, exhausted = false) {
  ...
  // Skor rendah setelah semua layer (termasuk deep review) tetap dinilai
  // "Tidak Sesuai" dengan skor & catatan apa adanya — tidak ada status
  // "Perlu Cek Manual" yang menggantung item.
