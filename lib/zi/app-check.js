  ...
  if (nameCheck.skip) {
    ...
    if (aiCheck.score < 40) {
      ...
      if (aiCheck.score < 40 && kriteriaText) {
        ...
        deepReview = await deepContentReview(files, fileContents, kriteriaText, id, readableFiles);
    ...
    if (!deepReview && kriteriaText && shouldSampleForQC(heuristic, aiCheck, aiCheck.score)) {
      ...
      const qcReview = await deepContentReview(files, fileContents, kriteriaText, id, readableFiles);
      if (qcReview) {
